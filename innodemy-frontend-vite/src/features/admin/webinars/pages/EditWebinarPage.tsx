...
import { useAdminWebinarQuery, useUpdateWebinarMutation } from "../hooks";
...
const EditWebinarPage = () => {
    ...
    const mutation = useUpdateWebinarMutation();
    ...
    // Prefill form once webinar loads
    useEffect(() => {
        if (!webinar) return;
        reset({
            title: webinar.title,
            description: webinar.description,
            date: toLocalDatetime(webinar.date),
            ...
            sectionOnePointsRaw: webinar.sectionOnePoints.join("\n"),
            sectionTwoTitle: webinar.sectionTwoTitle ?? "",
            sectionTwoPointsRaw: webinar.sectionTwoPoints.join("\n"),
        });
    }, [webinar, reset]);
    ...
    if (isLoading) {
        return <p className="p-6">Loading webinar...</p>;
    }
